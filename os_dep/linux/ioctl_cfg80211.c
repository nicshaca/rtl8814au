...
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,29))
	mon_ndev->netdev_ops = &rtw_cfg80211_monitor_if_ops;       //   <---   ENTRA QUI'
#else
...
static const struct ieee80211_iface_limit rtw_limits[] = {
	{
	        .max = 1,
...
static const struct ieee80211_iface_combination rtw_combinations[] = {
	{	.limits = rtw_limits,
...
static void rtw_cfg80211_preinit_wiphy(_adapter *adapter, struct wiphy *wiphy)
{
	...
	wiphy->interface_modes =	BIT(NL80211_IFTYPE_STATION)
								| BIT(NL80211_IFTYPE_ADHOC)       
                                                                | BIT(NL80211_IFTYPE_MESH_POINT)  // ADDED
                                                                #ifdef CONFIG_AP_MODE
								| BIT(NL80211_IFTYPE_AP)
								| BIT(NL80211_IFTYPE_AP_VLAN)     // ADDED
